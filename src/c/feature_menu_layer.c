...
void Event_Generator(MenuIndex *cell_index)
{
  ...
  Player.Trenchcoat.Drug[COCAINE].Price 	= (rand() % 1	+ 1	)		    ;
  Player.Trenchcoat.Drug[HEROINE].Price 	= (rand() % 1	+ 1	)		    ;
  Player.Trenchcoat.Drug[ACID]   .Price 	= (rand() % 1	+ 1	)		    ;
  Player.Trenchcoat.Drug[WEED]   .Price 	= (rand() % 1	+ 1	)		    ;
  Player.Trenchcoat.Drug[SPEED]  .Price	 	= (rand() % 1	+ 1	)		    ;
  Player.Trenchcoat.Drug[LUDES]  .Price	 	= (rand() % 1	+ 1	)		    ;
  ...
  switch(Player.Dice)
  {
    ...
    Player.Trenchcoat.Drug[HEROINE].Price = rand() % 99999999 + 99999999;
    break;

    case 6:
    case 7:
    toast_layer_show(message_layer, "HATERS MADE A BIG DOGE BUST!\nPRICES ARE OUTRAGEOUS!\n", SHORT_MESSAGE_DELAY, menu_header_heights[Player.MenuNumber]);
    Player.Trenchcoat.Drug[COCAINE].Price = rand() % 99999999 + 99999999;
    break;
    ...
    if (Player.Money.Cash >= Player.Trenchcoat.Guns[X].Price + 1
...
void Intro(MenuIndex *cell_index)
{
  ...
  Player.Money.Cash 							 						= 999999999;
  Player.Money.Balance				          		  = 999999999;
  Player.Money.Debt 							 					  = 0;

  Player.Trenchcoat.Price						          = 200;
  Player.Trenchcoat.Capacity	 				        = 999999999;
  Player.Trenchcoat.Freespace					        = 999999999;	

  Player.Trenchcoat.Drug[TOTAL].Price			    = 0;
  Player.Trenchcoat.Drug[TOTAL].Quantity		  = 0;

  Player.Trenchcoat.Drug[COCAINE].Price		    = rand() % 1 + 1;
  Player.Trenchcoat.Drug[COCAINE].Quantity	  = 99999;

  Player.Trenchcoat.Drug[HEROINE].Price		    = rand() % 1 + 1;
  Player.Trenchcoat.Drug[HEROINE].Quantity	  = 99999;

  Player.Trenchcoat.Drug[ACID].Price			    = rand() % 1 + 1;
  Player.Trenchcoat.Drug[ACID].Quantity		    = 99999;

  Player.Trenchcoat.Drug[WEED].Price			    = rand() % 1 + 1;
  Player.Trenchcoat.Drug[WEED].Quantity		    = 99999;

  Player.Trenchcoat.Drug[SPEED].Price			    = rand() % 1 + 1;
  Player.Trenchcoat.Drug[SPEED].Quantity	  	= 99999;

  Player.Trenchcoat.Drug[LUDES].Price			    = rand() % 1 + 1;
  Player.Trenchcoat.Drug[LUDES].Quantity	  	= 99999;

  Player.Trenchcoat.Guns[TOTAL].Quantity	  	= 3;

  Player.Trenchcoat.Guns[1].Price			      	= 1;
  Player.Trenchcoat.Guns[1].Damage		      	= 99;
  Player.Trenchcoat.Guns[1].Capacity					= 7;
  Player.Trenchcoat.Guns[1].Quantity					= 1;
  Player.Trenchcoat.Guns[1].Ammo							= 9999;

  Player.Trenchcoat.Guns[2].Price				      = 1;
  Player.Trenchcoat.Guns[2].Damage			      = 99;
  Player.Trenchcoat.Guns[2].Capacity		    	= 6;
  Player.Trenchcoat.Guns[2].Quantity		    	= 1;
  Player.Trenchcoat.Guns[2].Ammo			      	= 9999;

  Player.Trenchcoat.Guns[3].Price			      	= 1;
  Player.Trenchcoat.Guns[3].Damage			      = 99;
  Player.Trenchcoat.Guns[3].Capacity		    	= 9;
  Player.Trenchcoat.Guns[3].Quantity		    	= 1;
  Player.Trenchcoat.Guns[3].Ammo				      = 9999;
...
{
  if (Settings.vibrate) vibes_short_pulse();
  toast_layer_show(message_layer, "YOU GET HIGH ON THE MEMES, STUMBLE ON TO SOME TRACKS AND GET HIT BY A TRAIN!", LONG_MESSAGE_DELAY, menu_header_heights[0]);
  safe_free(confirm_header);
